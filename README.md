...
- локальный предпросмотр, пересъёмка и удаление фотографии без отправки на сервер.

Интерфейс камеры и предпросмотр уже реализованы. Распознавание текста по фотографии пока не подключено: после подтверждения фото приложение честно предлагает ручной ввод модели. Будущий OCR будет использовать тот же безопасный `/v1/fit`, а не определять совместимость самостоятельно.
...
│   ├── load_seed.py
│   └── validate_seed.py
├── tests/                            # unit-, camera- и integration-тесты
...
1. Подключить OCR к готовому интерфейсу камеры с ручным подтверждением неоднозначной модели.
